...
from utils.walter_utils import get_sensor_data

def get_config():
    """Returns reward config for walter environment."""
...
class WalterEnv(PipelineEnv):
    ...
    def __init__(self, 
        ...
        ROOT_PATH = epath.Path(__file__).parent.parent
        filepath = os.path.join(os.path.dirname(__file__), ROOT_PATH/'models/scene.xml')
        ...
        # Get sensor addresses used in training
    ...
    def step(self, state: State, action: jax.Array) -> State:
        ...
        state.info['pos_traj'] = jnp.roll(state.info['pos_traj'], 3).at[:3].set(x.pos[0][:3]) # Record state trajectory
    ...
    def _reward_pos_progress(self, x: Transform, last_xpos: jax.Array, command: jax.Array) -> jax.Array:
        """Reward when the robot is making progress along the command."""
        xpos = x.pos[0][:3]
